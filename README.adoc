...
- `@ComponentScan` tells Spring to look for other components, configurations, and services in the the `hello` package, allowing it to find, for example, a `HelloController` in a web application.

The `main()` method uses Spring Boot's `SpringApplication.run()` method to launch the application. Did you notice there
wasn't a single line of XML? No **web.xml** file either. This application is 100% pure Java and you didn't have to deal
...
However, the second call requesting the same quote took 0ms and resulted in a cache hit. This clearly shows that
the second call was cached and never actually hit the Quote service. But, when a final service call and specific,
non-cached quote request was made, it took 96 ms and result in a cache miss since this quote was not previously
in the cache.
